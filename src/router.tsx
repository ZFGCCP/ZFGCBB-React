...
const imported_routes = import.meta.glob("./pages/**/*.component.tsx", { import: "default" });
...
const regex_route_path_matches = /\/(src|pages|index|home|components|routes)|(\.tsx|\.component.tsx)$/g;
const regex_slug_matches = /\[\.{3}.+\]/;
const regex_slug_value_matches = /\[(.+)\]/;

const routes = Object.keys(imported_routes)
    .map((route) => {
        if (route === "./pages/home.component.tsx") return { element: <ContentView />, children: [{ path: "/", lazy: () => lazyLoad(route) }] };
        const path = route
